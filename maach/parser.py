...
class Parser:
    ...
    def statements(self):
        ...
        while True:
            ...
            # stop collecting block statements if we hit a block terminator
        ...
        return res.success(statements[0] if len(statements) == 1 else statements)
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "de"):
            ...
            if res.error:
                return res
            return res.success(ReturnNode(expr, token.pos_start, expr.pos_end))
    ...
    def expr(self):
        ...
        node = res.register(self.bin_op(
            self.comp_expr, ((TT_KEYWORD, "ebong"), (TT_KEYWORD, "othoba"))))
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'chol', int, float, identifier, '+', '-', '(', '[', 'jodi', 'ghorao', 'jotokhon', 'kaaj' or 'noy',",
                )
            )
