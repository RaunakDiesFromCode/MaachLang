"""
basic.py — compatibility wrapper

This file used to contain the full MaachLang interpreter. The implementation
was moved to `maach_core.py` to make the codebase more modular. This module
re-exports the main API (`run`, `global_symbol_table`) to preserve backward
compatibility with scripts and shells that import `basic`.
"""

import math

from maach.core import run as core_run, global_symbol_table as core_global_symbol_table

__all__ = ["run", "global_symbol_table"]
...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "chalao"):
            res.register_advancement()
            self.advance()
            return res.success(ContinueNode(pos_start, self.current_tok.pos_end.copy()))

        if self.current_tok.matches(TT_KEYWORD, "atkaao"):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'de', 'chalao', 'atkaao', 'chol', 'jodi', 'ghorao', 'jotokhon', 'kaaj', int, float, identifier, '+', '-', '(', '[' or 'noy'",
                )
            )
    ...
    def expr(self):
        ...
        node = res.register(
            self.bin_op(self.comp_expr,
                        ((TT_KEYWORD, "ebong"), (TT_KEYWORD, "othoba")))
        )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'chol', 'jodi', 'ghorao', 'jotokhon', 'kaaj', int, float, identifier, '+', '-', '(', '[' or 'noy'",
                )
            )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected int, float, identifier, '+', '-', '(', '[', 'jodi', 'ghorao', 'jotokhon', 'kaaj' or 'noy'",
                )
            )
    ...
    def atom(self):
        ...
        return res.failure(
            InvalidSyntaxError(
                tok.pos_start,
                tok.pos_end,
                "Expected int, float, identifier, '+', '-', '(', '[', jodi', 'ghorao', 'jotokhon', 'kaaj'",
            )
        )
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "othoba jodi"):
    ...
    def for_expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "barao"):
            res.register_advancement()
            self.advance()
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            return res.success(
                ForNode(var_name, start_value, end_value,
...
global_symbol_table.set("khali", Number.null)
global_symbol_table.set("bhul", Number.false)
global_symbol_table.set("thik", Number.true)
global_symbol_table.set("onko_pi", Number.pi)
global_symbol_table.set("onko_e", Number.e)
global_symbol_table.set("bol", BuiltInFunction.print)
...
run = core_run
global_symbol_table = core_global_symbol_table
